    ...
    <div id="editHeadPortrait">
        <n-modal v-model:show="showModal" class="custom-card" preset="card" :style="bodyStyle"
            ...
            <div v-show="showCropper">
                <VueCropper ref="cropper" id="vueCropper" :img="option.img" :outputSize="option.outputSize"
...
const okClick = () => {
    ...
    // 获取裁剪后的图片信息
    cropper.value.getCropBblo((data: any) => {
        // data 为裁剪图片的 Bblo 对象格式
        // console.log(data)
        const url = window.webkitURL.createObjectURL(data)
        // url 为裁剪图片的本地 host 链接格式
        // console.log(url)
    })
}
